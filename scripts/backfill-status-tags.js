// scripts/backfill-status-tags.js
// Backfills status_tags array with diagnostico_finalizado for all leads and
...
async function run(){
  console.log('\n🔧 Backfill status_tags');
  const PAGE = 1000;
  let from = 0; let total = 0; let updated = 0; let failures = 0;

  // Try to detect column existence quickly (best-effort)
  try {
    await client.from('quiz_leads').select('id, status_tags').limit(1);
  } catch (e) {
    console.error('❌ Column status_tags may not exist. Create it first, e.g.');
    console.error('   ALTER TABLE quiz_leads ADD COLUMN status_tags text[];');
    process.exit(1);
  }
  ...
  while (from < count) {
    ...
    for (const lead of data) {
      ...
      const updatedTags = merge(lead.status_tags, toAdd);
      try {
        await client.from('quiz_leads').update({ status_tags: updatedTags }).eq('id', lead.id);
        updated++;
      } catch (e) {
        failures++;
